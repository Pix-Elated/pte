    ...
    egui::Window::new("Item Properties")
        ...
        .show(ctx, |ui| {
            let Some((tx, ty)) = state.hover_tile else {
                ui.label(
                    egui::RichText::new("Hover a tile to edit item properties")
            ...
            ui.horizontal(|ui| {
                ui.label(
                    egui::RichText::new("House ID:")
                        .size(10.0)
                        .color(theme::TEXT_MUTED),
                );
                let mut hid = tile.house_id.map(|h| h as i64).unwrap_or(-1);
                if ui
                    .add(egui::DragValue::new(&mut hid).range(-1..=999999).speed(1))
                    .on_hover_text("-1 = no house")
                    .changed()
                {
            ...
            if tile.items.is_empty() && tile.ground.is_none() {
                ui.label(
                    egui::RichText::new("No items on this tile")
                        .size(10.0)
                        .color(theme::TEXT_MUTED),
                );
                return;
                            ...
                            egui::Frame::NONE
                                ...
                                .show(ui, |ui| {
                                    ...
                                    ui.horizontal(|ui| {
                                        ...
                                        let mut aid =
                                            item.action_id.map(|a| a as i32).unwrap_or(-1);
                                        if ui
                                            .add(
                                                egui::DragValue::new(&mut aid)
                                                    .range(-1..=65535)
                                                    .speed(1),
                                            )
                                            .on_hover_text("-1 = none")
                                            .changed()
                                        {
                                            item.action_id =
                                                if aid < 0 { None } else { Some(aid as u16) };
                                        }
                                    });
                                    ...
                                    ui.horizontal(|ui| {
                                        ...
                                        let mut uid =
                                            item.unique_id.map(|u| u as i32).unwrap_or(-1);
                                        if ui
                                            .add(
                                                egui::DragValue::new(&mut uid)
                                                    .range(-1..=65535)
                                                    .speed(1),
                                            )
                                            .on_hover_text("-1 = none")
                                        ...
                                        {
                                            item.unique_id =
                                                if uid < 0 { None } else { Some(uid as u16) };
                                        }
                                    });
                                        ...
                                        || item.id == 5765
                                    {
                                        ...
                                        ui.horizontal(|ui| {
                                            ui.label(
                                                egui::RichText::new("Teleport:")
                                                    .size(9.5)
                                                    .color(theme::TEXT_MUTED),
                                            );
                                            let mut dest =
                                                item.tele_dest.unwrap_or(pte_otbm::TeleportDest {
                                                    x: 0,
                                                    y: 0,
                                                    z: 7,
                                                });
